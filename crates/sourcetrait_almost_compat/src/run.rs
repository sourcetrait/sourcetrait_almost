...
fn dispatch(cli: Cli) -> CompatResult<()> {
    match cli.command {
        ...
        Command::Prompt {
            ...
            eprintln!("lmst: device {device:?}, dtype {dtype:?}");
            ...
            eprintln!("lmst: weights loaded in {:.1}s", load_start.elapsed().as_secs_f32());
...
fn pick_device(force_cpu: bool) -> CompatResult<candle_core::Device> {
    ...
    if r::candle::cuda_is_available() {
        ...
        eprintln!("lmst: cuda unavailable, running on cpu");
