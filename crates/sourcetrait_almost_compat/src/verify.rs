...
pub(crate) fn verify(
    ...
    eprintln!("lmst verify: weights loaded in {:.1}s", load_start.elapsed().as_secs_f32());
    ...
    if opts.cross_device {
        if matches!(device, candle_core::Device::Cpu) && dtype == candle_core::DType::F32 {
            eprintln!("lmst verify: skipping --cross-device (already on cpu f32)");
        } else {
            ...
            eprintln!("lmst verify: loading cpu f32 model for cross-device diff");
