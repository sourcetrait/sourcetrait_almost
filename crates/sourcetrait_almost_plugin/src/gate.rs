...
/// almost_check's whole body: `almost_check <typedef> [nuon-file]`
...
pub fn check_main() -> i32 {
    ...
    let (typedef, nuon_text) = match arguments.as_slice() {
        [typedef] => {
            ...
            if let Err(error) = io::stdin().read_to_string(&mut buffer) {
                eprintln!("almost_check: reading stdin failed: {error}");
                return 2;
            }
        ...
        [typedef, path] => match std::fs::read_to_string(path) {
            ...
            Err(error) => {
                eprintln!("almost_check: reading {path} failed: {error}");
                return 2;
            }
        },
        _ => {
            eprintln!("usage: almost_check <typedef> [nuon-file]  (absent file reads stdin)");
            return 2;
        }
    };
    ...
    match render_nuon(&report_value(&report)) {
        ...
        Err(error) => {
            eprintln!("almost_check: rendering the verdict failed: {error}");
            return 2;
        }
    }
