...
trait stays object-safe-adjacent and an implementation is not monomorphised
per caller. Nothing depends on that today; what it buys is that a future
boxed engine is a possible thing rather than a rewrite.
...
THE GENERIC THE API PROVIDES. A consumer takes `E: Era` and names a
concrete era bridge at exactly one instantiation point, so swapping eras is
one line rather than a sweep, and nothing in the consumer can reach past
this into an era's library because there is no path from here to one.
...
value is itself `Send` while the value is not, so `engine` can cross to the
thread that will own its result. An `Era` carrying an already-built engine
could not be moved to that thread at all.
...
is about to open before paying for the model. Its answer and `Engine::open`'s
can legitimately differ - one is the era's constant and the other is what
actually loaded - and a configuration pointing at another checkpoint is
exactly that case.
