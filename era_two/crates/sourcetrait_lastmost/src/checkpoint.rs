...
pub(crate) fn data_home() -> LastmostResult<PathBuf> {
...
pub(crate) fn resolve_model_dir(pick: ModelPick) -> LastmostResult<PathBuf> {
    Ok(data_home()?
...
pub(crate) fn read_safetensors_header(path: &Path) -> LastmostResult<Vec<TensorInfo>> {
...
pub(crate) fn read_config(model_dir: &Path) -> LastmostResult<serde_json::Value> {
...
/// (id, content, special).
pub(crate) fn read_added_tokens(model_dir: &Path) -> LastmostResult<Vec<(u64, String, bool)>> {
