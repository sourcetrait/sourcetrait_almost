...
//! refquest oracle (C2) cpu-f32 references (all #[ignore] - they need
//! the local DPO checkpoint and a reference env var), plus the
//! chunked-vs-stateless equivalence.
//!
//! Env: QUEST_STATELESS_REFERENCE points at the short single-mode reference
//! file (the StatelessGate); QUEST_ORACLE_DUMPS_DIR points at the oracle
...
fn oracle_dumps_dir() -> String {
    env::var("QUEST_ORACLE_DUMPS_DIR")
        .expect("QUEST_ORACLE_DUMPS_DIR must point at the oracle dumps root (short/mid/long)")
}

#[test]
#[ignore = "needs the DPO checkpoint + QUEST_ORACLE_DUMPS_DIR"]
...
#[test]
#[ignore = "needs the DPO checkpoint + QUEST_ORACLE_DUMPS_DIR; long runtime"]
...
/// rows (the refquest oracle internal-spread envelope; their bf16 rows run
...
#[test]
#[cfg(feature = "cuda")]
#[ignore = "needs the DPO checkpoint + QUEST_ORACLE_DUMPS_DIR + a cuda card"]
...
#[test]
#[cfg(feature = "cuda")]
#[ignore = "diagnostic: needs the DPO checkpoint + QUEST_ORACLE_DUMPS_DIR + a cuda card"]
