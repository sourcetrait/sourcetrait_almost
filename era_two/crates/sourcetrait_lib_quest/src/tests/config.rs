...
fn snapshots_dir_defaults_to_the_cache_home_and_overrides_by_file() {
    let config = LibConfig::default();
    assert!(
        config.snapshots_dir.ends_with("sourcetrait/quest/snapshots"),
        "embedded default: {:?}",
...
fn embedded_defaults_carry_the_card_posture() {
    ...
    assert!(settings.fused_prefill, "PrefillDispatch defaults on");
...
fn eviction_score_knobs_default_and_override() {
    ...
    let eviction = settings.eviction.expect("armed");
    assert_eq!(
        eviction.score_tail, 16,
        "the adopted RescoreTuning default"
    );
