...
fn field<'a>(record: &'a lib::nu::Record, name: &str) -> BquestResult<&'a lib::nu::Value> {
...
fn field_str(record: &lib::nu::Record, name: &str) -> BquestResult<String> {
...
fn field_int(record: &lib::nu::Record, name: &str) -> BquestResult<i64> {
...
fn field_ids(record: &lib::nu::Record, name: &str) -> BquestResult<Vec<u32>> {
    let value = field(record, name)?;
    let list = match value.as_list() {
        Ok(list) => list,
        Err(e) => snafu::whatever!("field {name}: {e}"),
...
fn field_strings(record: &lib::nu::Record, name: &str) -> BquestResult<Vec<String>> {
    let value = field(record, name)?;
    let list = match value.as_list() {
        Ok(list) => list,
        Err(e) => snafu::whatever!("field {name}: {e}"),
