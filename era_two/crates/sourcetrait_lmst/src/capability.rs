...
const CAPABILITY_HOME_RELATIVE: &str = "sourcetrait/almost/capability";
...
fn data_home() -> LmstResult<PathBuf> {
...
fn encode(tokenizer: &tokenizers::Tokenizer, text: &str) -> LmstResult<Vec<u32>> {
...
fn ids_tensor(ids: &[u32], device: &candle_core::Device) -> LmstResult<candle_core::Tensor> {
...
pub(crate) fn capability_run(cli: &Cli, args: &CapabilityRunArgs) -> LmstResult<()> {
...
fn collect_request_files(dir: &Path, into: &mut Vec<PathBuf>) -> LmstResult<()> {
...
fn read_rows(file: &Path) -> LmstResult<Vec<RequestRow>> {
