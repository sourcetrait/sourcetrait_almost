...
fn v_int(value: i64) -> lib::nu::Value {
    lib::nu::Value::int(value, span())
}

fn v_float(value: f64) -> lib::nu::Value {
    lib::nu::Value::float(value, span())
}

fn v_str(value: &str) -> lib::nu::Value {
    lib::nu::Value::string(value, span())
}

fn v_bool(value: bool) -> lib::nu::Value {
    lib::nu::Value::bool(value, span())
}

fn v_int_list(ids: &[u32]) -> lib::nu::Value {
    lib::nu::Value::list(ids.iter().map(|id| v_int(*id as i64)).collect(), span())
}

fn field<'a>(record: &'a lib::nu::Record, name: &str) -> LmstResult<&'a lib::nu::Value> {
...
fn field_str(record: &lib::nu::Record, name: &str) -> LmstResult<String> {
...
fn field_int(record: &lib::nu::Record, name: &str) -> LmstResult<i64> {
...
fn field_rows<'a>(
    ...
    name: &str,
) -> LmstResult<Vec<&'a lib::nu::Record>> {
...
fn field_ids(record: &lib::nu::Record, name: &str) -> LmstResult<Vec<u32>> {
    let value = field(record, name)?;
    let list = match value.as_list() {
        Ok(list) => list,
        Err(e) => snafu::whatever!("field {name}: {e}"),
...
fn field_strings(record: &lib::nu::Record, name: &str) -> LmstResult<Vec<String>> {
    let value = field(record, name)?;
    let list = match value.as_list() {
        Ok(list) => list,
        Err(e) => snafu::whatever!("field {name}: {e}"),
...
fn read_plans(value: &lib::nu::Value) -> LmstResult<Vec<TranscriptPlan>> {
...
pub(crate) fn speculate_record(cli: &Cli, args: &SpeculateRecordArgs) -> LmstResult<()> {
        ...
        lib::nu::record! {
            ...
            "meta" => lib::nu::Value::record(
                lib::nu::record! {
                    ...
                    "lmst_version" => v_str(env!("CARGO_PKG_VERSION")),
...
fn read_transcript(path: &Path) -> LmstResult<RecordedTranscript> {
...
fn read_phases(path: &Path) -> LmstResult<Vec<PhaseRow>> {
    let value = lib::nu::load_value(path)?;
...
pub(crate) fn speculate_simulate(args: &SpeculateSimulateArgs) -> LmstResult<()> {
...
pub(crate) fn speculate_tokens(cli: &Cli, args: &SpeculateTokensArgs) -> LmstResult<()> {
    let config = lib::LibConfig::load_from_dir(cli.dir.as_ref(), cli.config.as_deref())?;
