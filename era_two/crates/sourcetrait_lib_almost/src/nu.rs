...
pub fn parse_typedef(typedef: &str) -> LibAlmostResult<Type> {
...
pub fn from_nuon_text(text: &str) -> LibAlmostResult<Value> {
...
/// stringify.
pub fn to_nuon_text(value: &Value) -> LibAlmostResult<String> {
...
/// vs derived.
pub fn conform(value: &Value, declared: &Type) -> LibAlmostResult<()> {
...
/// per file).
pub fn load_value(path: &Path) -> LibAlmostResult<Value> {
    from_nuon_text(&fs::read_to_string(path)?)
}

/// Write one whole-value .nuon file, newline-terminated (parent
/// directories created).
pub fn save_value(path: &Path, value: &Value) -> LibAlmostResult<()> {
...
/// skip.
pub fn load_lines(path: &Path) -> LibAlmostResult<Vec<Value>> {
...
pub fn save_lines(path: &Path, values: &[Value]) -> LibAlmostResult<()> {
...
pub fn append_line(path: &Path, value: &Value) -> LibAlmostResult<()> {
...
fn render_line(value: &Value) -> LibAlmostResult<String> {
    let text = to_nuon_text(value)?;
    snafu::ensure_whatever!(
        !text.contains('\n'),
...
fn ensure_parent(path: &Path) -> LibAlmostResult<()> {
