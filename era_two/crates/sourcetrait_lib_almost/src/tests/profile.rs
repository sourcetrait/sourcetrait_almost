...
/// Env: ALMOST_NEEDLE_SPEC + ALMOST_PROFILE_OUT; optional
/// ALMOST_PROFILE_LENGTHS (csv, default 16384,24576,32768),
/// ALMOST_PROFILE_DEPTHS (csv, default 25,75), ALMOST_PROFILE_WINDOW
/// (default 4096).
#[test]
#[cfg(feature = "cuda")]
#[ignore = "the A2 battery: needs the DPO checkpoint + a cuda card + ALMOST_NEEDLE_SPEC/_PROFILE_OUT"]
fn attn_profile_battery() {
    use crate::needle::NeedleRig;

    let spec_path = env::var("ALMOST_NEEDLE_SPEC").expect("ALMOST_NEEDLE_SPEC");
    let out_path = env::var("ALMOST_PROFILE_OUT").expect("ALMOST_PROFILE_OUT");
    let lengths: Vec<usize> = env::var("ALMOST_PROFILE_LENGTHS")
        .unwrap_or_else(|_| "16384,24576,32768".to_string())
        .split(',')
        .map(|token| token.trim().parse().expect("length"))
        .collect();
    let depths: Vec<usize> = env::var("ALMOST_PROFILE_DEPTHS")
        .unwrap_or_else(|_| "25,75".to_string())
        .split(',')
        .map(|token| token.trim().parse().expect("depth"))
        .collect();
    let window: usize = env::var("ALMOST_PROFILE_WINDOW")
