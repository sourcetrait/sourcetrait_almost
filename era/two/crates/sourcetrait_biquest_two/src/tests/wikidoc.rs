...
/// One field of a record row, as a string.
fn field_text<'a>(row: &'a crate::harness::nu::Value, name: &str) -> &'a str {
    row.as_record()
        .expect("row is a record")
        .get(name)
        .unwrap_or_else(|| panic!("row carries {name}"))
        .as_str()
        .unwrap_or_else(|_| panic!("{name} is a string"))
}

#[test]
fn the_record_folds_the_document_structure() {
    let document = render_word_document(&[page("dog", PAGE_TEXT)]).expect("render");
    let record = assemble_record("dog", "dog", &document.nodes);
    let record = record.as_record().expect("record");
    assert_eq!(record.get("word").expect("word").as_str().expect("str"), "dog");
    assert_eq!(record.get("title").expect("title").as_str().expect("str"), "dog");
    let sections = record
        .get("sections")
        .expect("sections")
        .as_list()
        .expect("sections list");
    let names: Vec<&str> = sections
        .iter()
        .map(|section| field_text(section, "name"))
        .collect();
    assert_eq!(names, vec!["Etymology", "Noun", "Anagrams"]);

    let etymology = sections[0].as_record().expect("record");
    let blocks = etymology
        .get("blocks")
        .expect("blocks")
        .as_list()
        .expect("blocks list");
    assert_eq!(field_text(&blocks[0], "kind"), "prose");
    assert_eq!(field_text(&blocks[0], "text"), "A blend of [alpha] + [beta].");
    assert!(etymology.get("senses").expect("senses").is_nothing());
    assert!(etymology.get("head").expect("head").is_nothing());

    let noun = sections[1].as_record().expect("record");
    assert_eq!(noun.get("head").expect("head").as_str().expect("str"), "dog");
    assert_eq!(
        noun.get("head_line").expect("head_line").as_str().expect("str"),
        "(plural [dogs])"
    );
    let senses = noun
        .get("senses")
        ...
        .as_list()
        .expect("senses list");
    let sense = senses[0].as_record().expect("record");
    assert_eq!(sense.get("depth").expect("depth").as_int().expect("int"), 1);
    assert_eq!(
        sense.get("gloss").expect("gloss").as_str().expect("str"),
        "(informal) A [pet], with [friends](friend)."
    );
    let quotes = sense
        .get("quotes")
        .expect("quotes")
        ...
        .expect("quotes list");
    assert_eq!(
        quotes[0].as_str().expect("str"),
        "*Dogs*: \"the **dog** ran\""
    );
    let subsections = noun
        .get("subsections")
        .expect("subsections")
        .as_list()
        .expect("subsections list");
    let subsection_names: Vec<&str> = subsections
        .iter()
        .map(|subsection| field_text(subsection, "name"))
        .collect();
    assert_eq!(subsection_names, vec!["Synonyms", "Derived terms"]);
    // A pure anchor-list item stores its bare word (TheUser's ruling);
    // the markdown keeps the anchor form.
    let synonym_blocks = subsections[0]
        .as_record()
        .expect("record")
        .get("blocks")
        .expect("blocks")
        .as_list()
        .expect("blocks list");
    let synonym_items: Vec<&str> = synonym_blocks
        .iter()
        .map(|block| field_text(block, "text"))
        .collect();
    assert_eq!(synonym_items, vec!["hound", "pooch"]);
    assert!(document.markdown.contains("- [hound]"));

    let anagrams = sections[2].as_record().expect("record");
    let anagram_blocks = anagrams
        .get("blocks")
        .expect("blocks")
        .as_list()
        .expect("blocks list");
    assert_eq!(field_text(&anagram_blocks[0], "text"), "god");
}

#[test]
fn the_record_keeps_nested_sense_depths_flat() {
    let text = "==English==\n\n===Noun===\n{{en-noun}}\n\n# A [[gentle]] incline.\n## {{lb|en|military}}\n### A fortification incline.\n# A second sense.\n";
    let document = render_word_document(&[page("glacis", text)]).expect("render");
    let record = assemble_record("glacis", "glacis", &document.nodes);
    let sections = record
        .as_record()
        .expect("record")
        .get("sections")
...
fn embedded_newlines_flatten_to_spaces() {
    ...
    let document = render_word_document(&[page("x", text)]).expect("render");
    assert!(
        document.markdown.contains("A (split across) case."),
        "markdown: {}",
        document.markdown
    );
    assert!(document
