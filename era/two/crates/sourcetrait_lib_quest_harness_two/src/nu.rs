...
pub fn parse_typedef(typedef: &str) -> HarnessQuestResult<Type> {
...
pub fn from_nuon_text(text: &str) -> HarnessQuestResult<Value> {
...
pub fn to_nuon_text(value: &Value) -> HarnessQuestResult<String> {
    render_nuon(value, nuon::ToStyle::Default)
}

/// Value -> CONDENSED NUON, what `to nuon --raw` emits.
pub fn to_nuon_condensed(value: &Value) -> HarnessQuestResult<String> {
    render_nuon(value, nuon::ToStyle::Raw)
}

/// Value -> PRETTY NUON, two-space indented as `to nuon --pretty`.
pub fn to_nuon_pretty(value: &Value) -> HarnessQuestResult<String> {
    render_nuon(value, nuon::ToStyle::Spaces(2))
}

fn render_nuon(value: &Value, style: nuon::ToStyle) -> HarnessQuestResult<String> {
...
pub fn conform(value: &Value, declared: &Type) -> HarnessQuestResult<()> {
...
/// per file).
pub fn load_value(path: &Path) -> HarnessQuestResult<Value> {
    from_nuon_text(&fs::read_to_string(path)?)
}

/// Write one whole-value .nuon file, newline-terminated.
pub fn save_value(path: &Path, value: &Value) -> HarnessQuestResult<()> {
...
/// skip.
pub fn load_lines(path: &Path) -> HarnessQuestResult<Vec<Value>> {
...
pub fn save_lines(path: &Path, values: &[Value]) -> HarnessQuestResult<()> {
...
pub fn append_line(path: &Path, value: &Value) -> HarnessQuestResult<()> {
...
fn render_line(value: &Value) -> HarnessQuestResult<String> {
    let text = to_nuon_text(value)?;
    snafu::ensure_whatever!(
        !text.contains('\n'),
...
fn ensure_parent(path: &Path) -> HarnessQuestResult<()> {
