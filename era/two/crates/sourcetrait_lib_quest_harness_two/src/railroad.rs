...
impl TrainNom {
    ...
    pub fn fresh() -> HarnessQuestResult<Self> {
...
impl Railroad {
    ...
    pub fn home() -> HarnessQuestResult<PathBuf> {
    ...
    pub fn lay() -> HarnessQuestResult<Self> {
        Self::lay_in(&Self::home()?)
    }

    /// Lay one under a root of the caller's choosing.
    pub fn lay_in(root: &Path) -> HarnessQuestResult<Self> {
    ...
    pub fn at(dir: &Path) -> HarnessQuestResult<Self> {
    ...
    pub fn commit(&self) -> HarnessQuestResult<usize> {
    ...
    fn next_rev(&self) -> HarnessQuestResult<usize> {
...
pub fn revision_of(dir: &Path) -> HarnessQuestResult<Revision> {
    snafu::ensure_whatever!(
        dir.is_dir(),
...
fn identity_ready(root: &Path) -> HarnessQuestResult<()> {
...
fn git(dir: &Path, args: &[&str]) -> HarnessQuestResult<String> {
    let output = process::Command::new("git")
        .current_dir(dir)
        .args(args)
        .output()?;
