...
`columns`, `sort-by` and `drop` arrived the way `split row` did: the
data-transform syllabus asks `evaluate` for a record's field names and a
table's largest row, the pipe-transform syllabus asks it to remove a last
entry, and a trained transform whose head is unregistered dies as an
implicit external at serve. The difference is that these were measured in
from the training set before the live failure rather than after it. The
vocabulary lock in the tests is the standing check: every head the
syllabus teaches must resolve here, so the set follows the syllabus rather
than being curated on taste.
