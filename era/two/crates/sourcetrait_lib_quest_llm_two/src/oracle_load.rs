...
pub struct HybridCheckpointConfig {
...
impl HybridCheckpointConfig {
    pub fn load(model_dir: &Path) -> LibQuestResult<Self> {
    ...
    pub fn is_gdn_layer(&self, index: usize) -> LibQuestResult<bool> {
...
impl HybridWeights {
    ...
    pub fn from_tensors(tensors: HashMap<String, (Vec<usize>, Vec<f32>)>) -> Self {
        Self { tensors }
    }
    ...
    pub fn take(&mut self, name: &str) -> LibQuestResult<(Vec<usize>, Vec<f32>)> {
    ...
    pub fn take_linear_transposed<B: burn::tensor::backend::Backend>(
        &mut self,
        name: &str,
        device: &B::Device,
    ) -> LibQuestResult<burn::tensor::Tensor<B, 2>> {
    ...
    pub fn take_vector<B: burn::tensor::backend::Backend>(
        &mut self,
        name: &str,
        device: &B::Device,
    ) -> LibQuestResult<burn::tensor::Tensor<B, 1>> {
        let (shape, values) = self.take(name)?;
    ...
    pub fn take_row<B: burn::tensor::backend::Backend>(
        &mut self,
        name: &str,
        device: &B::Device,
    ) -> LibQuestResult<burn::tensor::Tensor<B, 2>> {
    ...
    pub fn take_conv_taps<B: burn::tensor::backend::Backend>(
        &mut self,
        name: &str,
        kernel: usize,
        device: &B::Device,
    ) -> LibQuestResult<Vec<burn::tensor::Tensor<B, 2>>> {
        let (shape, values) = self.take(name)?;
        snafu::ensure_whatever!(
    ...
    pub fn take_host_matrix(
        &mut self,
        name: &str,
    ) -> LibQuestResult<(usize, usize, Vec<f32>)> {
        let (shape, values) = self.take(name)?;
        snafu::ensure_whatever!(shape.len() == 2, "{name}: expected rank 2, got {shape:?}");
...
pub fn dump_read_u32(
    file: &safetensors::SafeTensors,
    name: &str,
) -> LibQuestResult<Vec<u32>> {
