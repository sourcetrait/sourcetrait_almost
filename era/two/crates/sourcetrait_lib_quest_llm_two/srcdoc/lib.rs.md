...
triangle; `train` stacks autodiff; `train-cuda`/`burn-cuda` are the two
cuda grades and never combine with each other. `cuda`+`train-cuda` in one
binary is impossible (cudarc nccl unification), which is why bquest
builds twice.
