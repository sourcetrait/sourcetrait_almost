...
pub(crate) enum Command {
    /// Replay a refquest dump's token ids through the burn model, writing a
    /// dump in the same format
    Dump {
        /// refquest parity dump supplying prompt_ids + fed_ids
        #[arg(long)]
        ids_from: PathBuf,
        /// Output dump path
        #[arg(long)]
...
/// Mirrors refquest's default layout so the two binaries share a checkpoint:
