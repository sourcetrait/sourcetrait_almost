...
/// E2 saved-context home: $XDG_CACHE_HOME/sourcetrait/quest/snapshots
...
/// Chat session logs: $XDG_CACHE_HOME/sourcetrait/quest/session.
...
pub fn ensure_model(model_id: &str, dir: &Path) -> QuestResult<ModelPaths> {
    fn fetch(
        ...
        eprintln!("quest: pulling {filename}");
        let path = repo
            .download_file()
            .filename(filename)
