...
fn kernel(
    ...
    name: &str,
) -> QuestResult<cudarc::driver::CudaFunction> {
...
impl DecodeStage {
    ...
    /// buffers reallocate.
    pub(crate) fn rearm(&mut self, full_len: usize, ring_len: usize) -> QuestResult<()> {
    ...
    pub(crate) fn reset_full_valid(&self, full_len: usize) -> QuestResult<()> {
    ...
    pub(crate) fn ensure_buckets(&mut self, full_len: usize, ring_len: usize) -> QuestResult<()> {
        let full_bucket = bucket_for(full_len + 1);
