...
/// defaults). Profiles live under $XDG_CONFIG_HOME/sourcetrait/quest/:
...
/// Chat-surface intent (camp). log defaults ON: each turn rewrites
/// <cache>/sourcetrait/quest/session/<session nom>.txt.
...
/// $XDG_CONFIG_HOME/sourcetrait/quest (XDG spec fallback ~/.config).
fn profiles_home() -> PathBuf {
    ...
    base.join("sourcetrait").join("quest")
}
...
fn parse_config(path: &Path) -> QuestResult<Config> {
...
fn parse_settings_patch(path: &Path) -> QuestResult<SettingsPatch> {
    let raw = std::fs::read_to_string(path)?;
...
pub fn load_config(arg: Option<&str>) -> QuestResult<(Config, Option<String>)> {
    match arg {
        None => {
...
pub fn resolve_device(config: &Config) -> QuestResult<candle_core::Device> {
    match config.device {
