...
impl<'t> TokenStream<'t> {
    ...
    pub(crate) fn next_token(&mut self, token: u32) -> AlmostResult<Option<String>> {
    ...
    pub(crate) fn decode_rest(&self) -> AlmostResult<Option<String>> {
        let previous_text = self.decode(&self.tokens[self.prev_index..self.current_index])?;
        let text = self.decode(&self.tokens[self.prev_index..])?;
    ...
    fn decode(&self, ids: &[u32]) -> AlmostResult<String> {
