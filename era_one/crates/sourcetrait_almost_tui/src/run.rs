...
/// talmost: the interactive terminal chat (thin driver over the lib).
#[derive(Debug, clap::Parser)]
#[command(name = "talmost", version, about)]
struct Cli {
...
pub fn run() {
    ...
    if let Err(error) = drive(cli) {
        eprintln!("talmost: {error}");
        std::process::exit(1);
    }
}

fn drive(cli: Cli) -> lib::AlmostResult<()> {
    ...
    if settings.graph {
        eprintln!("talmost: chained chat runs capture-off (graph profile setting ignored)");
        settings.graph = false;
    }
    let device = lib::resolve_device(&config)?;
    let dtype = lib::resolve_dtype(&config, &device);
    eprintln!(
        "talmost: device {device:?}, dtype {dtype:?}{}",
    ...
    eprintln!("talmost: weights loaded in {:.1}s", loaded.load_seconds);
