...
fn encode_len(tokenizer: &tokenizers::Tokenizer, text: &str) -> QuestResult<usize> {
...
pub fn needle(
    ...
    eprintln!(
        "quest needle: weights loaded in {:.1}s; device {device:?}, dtype {dtype:?}",
    ...
    for &multi in modes {
        for &length in lengths {
            ...
            eprintln!(
                "quest needle: {} {} done - {hits}/{cases} ({:.0}s elapsed)",
    ...
    if let Some(path) = &opts.out {
        ...
        eprintln!("quest needle: results written to {}", path.display());
    }
    ...
    if let Some(path) = &opts.profile_out {
        ...
        std::fs::write(path, serde_json::to_string_pretty(&payload)?)?;
        eprintln!("quest needle: attention profile written to {}", path.display());
    }
    Ok(())
}
