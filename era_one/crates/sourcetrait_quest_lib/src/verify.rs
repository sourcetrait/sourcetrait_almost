...
pub fn verify(
    ...
    eprintln!(
        "quest verify: {n} tokens, device {device:?}, dtype {dtype:?}, mode {}",
    ...
    eprintln!("quest verify: weights loaded in {:.1}s", load_start.elapsed().as_secs_f32());
    ...
    let snapshot_path = std::env::temp_dir().join(format!(
        "quest_verify_snapshot_{}.safetensors",
        std::process::id()
    ));
    ...
    if opts.cross_device {
        if matches!(device, candle_core::Device::Cpu) && dtype == candle_core::DType::F32 {
            eprintln!("quest verify: skipping --cross-device (already on cpu f32)");
        } else {
            ...
            eprintln!("quest verify: loading cpu f32 model for cross-device diff");
...
fn build_ids(tokenizer: &tokenizers::Tokenizer, long: bool, window: usize) -> QuestResult<Vec<u32>> {
    let text = if long {
...
fn tensor_ids(ids: &[u32], device: &candle_core::Device) -> QuestResult<candle_core::Tensor> {
...
/// pass).
pub fn verify_graph(paths: &ModelPaths) -> QuestResult<()> {
    snafu::ensure_whatever!(
    ...
    eprintln!(
        "quest verify-graph: weights loaded in {:.1}s; {GRAPH_CHECK_STEPS} steps per leg",
    ...
    for &target in &[32768usize, 2048] {
        ...
        eprintln!(
            "quest verify-graph: {target}-token legs done ({:.0}s elapsed)",
    ...
    eprintln!(
        "quest verify-graph: eviction epoch legs done ({:.0}s elapsed)",
        leg_start.elapsed().as_secs_f32()
    );
    ...
    drop(model);
    {
        ...
        eprintln!(
            "quest verify-graph: chained generate_from leg done ({:.0}s elapsed)",
            leg_start.elapsed().as_secs_f32()
        );
...
fn chained_prompt(tokenizer: &tokenizers::Tokenizer) -> QuestResult<String> {
