...
impl AttnMask {
    ...
    fn eager_tensor(&self) -> QuestResult<Option<&candle_core::Tensor>> {
        match self {
...
fn ring_linear_span(
    ...
    logical_start: usize,
    count: usize,
) -> QuestResult<candle_core::Tensor> {
...
/// The quest Olmo 3 decoder (D1): HF-shaped single-target module, MHA
...
impl Model {
    pub fn new(cfg: &Olmo3Config, settings: Settings, vb: candle_nn::VarBuilder) -> QuestResult<Self> {
        snafu::ensure_whatever!(
            cfg.num_kv_heads() == cfg.num_attention_heads,
            "quest supports MHA only (sole-checkpoint policy): num_key_value_heads {} != num_attention_heads {}",
            cfg.num_kv_heads(),
    ...
    pub fn forward(&mut self, input_ids: &candle_core::Tensor, seqlen_offset: usize) -> QuestResult<candle_core::Tensor> {
    ...
    pub fn forward_all(&mut self, input_ids: &candle_core::Tensor, seqlen_offset: usize) -> QuestResult<candle_core::Tensor> {
    ...
    pub(crate) fn reserve_full_caches(&mut self, total_len: usize) -> QuestResult<()> {
        #[cfg(feature = "cuda")]
        {
    ...
    /// sliding layer.
    pub(crate) fn cache_mark(&self, offset: usize, span: usize) -> QuestResult<CacheMark> {
        snafu::ensure_whatever!(
    ...
    pub(crate) fn compact_full_caches(&mut self) -> QuestResult<()> {
    ...
    #[cfg(feature = "cuda")]
    pub(crate) fn arm_graph_decode(&mut self, expected_total: usize) -> QuestResult<()> {
        snafu::ensure_whatever!(
            self.device.is_cuda(),
    ...
    #[cfg(not(feature = "cuda"))]
    pub(crate) fn arm_graph_decode(&mut self, _expected_total: usize) -> QuestResult<()> {
    ...
    #[cfg(feature = "cuda")]
    pub(crate) fn set_graph_capture(&mut self, enabled: bool) -> QuestResult<()> {
    ...
    #[cfg(not(feature = "cuda"))]
    pub(crate) fn set_graph_capture(&mut self, _enabled: bool) -> QuestResult<()> {
        snafu::whatever!("this build carries no cuda support (decode graphs need --features cuda)")
    }
    ...
    #[cfg(feature = "cuda")]
    pub(crate) fn graph_disarm_when_full(&mut self) -> QuestResult<()> {
        if !self.graph_armed() {
            return Ok(());
        }
        let capacity = self
    ...
    #[cfg(feature = "cuda")]
    fn graph_type_state(&self) -> QuestResult<(usize, usize, usize)> {
        let full = self
            .layers
            .iter()
            .find(|layer| layer.layer_type == LayerType::Full);
    ...
    ) -> QuestResult<cudarc::driver::CudaGraph> {
    ...
    #[cfg(feature = "cuda")]
    fn graph_forward_sequence(&mut self, stage: &graph::DecodeStage) -> QuestResult<()> {
    ...
    pub(crate) fn cache_rollback(&mut self, mark: &CacheMark, accepted: usize) -> QuestResult<()> {
        snafu::ensure_whatever!(
...
impl Attention {
    ...
    ) -> QuestResult<(candle_core::Tensor, candle_core::Tensor)> {
    ...
    /// slot.
    fn cache_mark(&self, offset: usize, span: usize) -> QuestResult<AttentionMark> {
    ...
    fn export_cache(&self) -> QuestResult<Option<(candle_core::Tensor, candle_core::Tensor)>> {
        let cpu = candle_core::Device::Cpu;
...
impl Mlp {
    fn new(cfg: &Olmo3Config, vb: candle_nn::VarBuilder) -> QuestResult<Self> {
    ...
    fn forward(&self, xs: &candle_core::Tensor) -> QuestResult<candle_core::Tensor> {
