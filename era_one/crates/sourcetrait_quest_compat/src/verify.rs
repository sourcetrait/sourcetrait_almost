...
pub(crate) struct VerifyOptions {
    /// Use a prompt longer than the sliding window, so the decode slice
    /// path and the banded window mask are both live.
    pub(crate) long: bool,
...
/// Self-consistency battery: every check compares two code paths of the SAME
/// implementation, so agreement bars are tight. Exits with an error when a
/// same-device check exceeds its bar; cross-device numbers are reported only.
pub(crate) fn verify(
    paths: &ModelPaths,
    ...
    eprintln!(
        "refquest verify: {n} tokens, device {device:?}, dtype {dtype:?}, mode {}",
    ...
    eprintln!("refquest verify: weights loaded in {:.1}s", load_start.elapsed().as_secs_f32());
    ...
    // Chunked prefill vs single (offset-aware banded masks).
    ...
    // sliding-layer slice path vs the banded mask).
    ...
    if opts.cross_device {
        if matches!(device, candle_core::Device::Cpu) && dtype == candle_core::DType::F32 {
            eprintln!("refquest verify: skipping --cross-device (already on cpu f32)");
        } else {
            ...
            eprintln!("refquest verify: loading cpu f32 model for cross-device diff");
            let cpu = candle_core::Device::Cpu;
            let vb_cpu = unsafe {
                candle_nn::VarBuilder::from_mmaped_safetensors(&paths.shards, candle_core::DType::F32, &cpu)?
            };
            let mut model_cpu = Model::new(&config, vb_cpu)?;
...
fn build_ids(tokenizer: &tokenizers::Tokenizer, long: bool, window: usize) -> CompatResult<Vec<u32>> {
    let text = if long {
    ...
    if long {
        // window + 256: enough past-window positions to exercise the decode
        // slice path and the banded mask, small enough that the quadratic
        // attention transient (heads * n^2, plus its f32 softmax copy) fits
        // Tier-A VRAM beside the weights.
...
fn tensor_ids(ids: &[u32], device: &candle_core::Device) -> CompatResult<candle_core::Tensor> {
